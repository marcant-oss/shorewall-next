    ...
    "cd /root/shorewall-nft && \
     systemd-run --unit=shorewalld-smoke --collect \
       .venv/bin/shorewalld --listen-prom :9748 \
                            --scrape-interval 5"
sleep 2
ssh root@192.168.203.83 "curl -s http://localhost:9748/metrics | \
    grep shorewall_nft_ | head -20"
ssh root@192.168.203.83 "systemctl stop shorewalld-smoke"
...
ssh root@192.168.203.83 "systemctl stop shorewalld-multi"
