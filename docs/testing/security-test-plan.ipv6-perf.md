...
- **Scenario**: `conn_storm_direct` targeting the reference HA firewall's SSHd
  (`217.14.160.75:22`) from a native-mode endpoint in the `net` zone backbone
  (`217.14.160.64/27`, tester01 eth2).
- **Rule used**: `SSH(ACCEPT) net:217.14.160.64/27 $FW` — line 47 of the reference
  HA firewall's `/etc/shorewall/rules`.  No FW config change is required.
- **Traffic path**: tester01 eth2 (`217.14.160.74/27`) → FW `bond1`
  (`217.14.160.75`) tcp/22 → conntrack ESTABLISHED in the `net → $FW` INPUT chain.
...
- **Operator caveat**: the sim-uplink OSPF netns (router ID `217.14.160.77`) also
