...
tester01 eth1.20 (wan-native)  ←— VLAN 20 L2 —→  tester02 eth1.20 (lan-downstream)
  217.14.168.253/24                                  217.14.168.254/24
  2a00:f88:0:3168::253/64                            2a00:f88:0:3168::254/64
```
...
`source_role: wan-native` (tester01 eth1.20, IPv4 `217.14.168.253/24`) in
...
`source_role: wan-native` (tester01 eth1.20 with real IPv4 `217.14.168.253/24`)
...
for h in 192.168.203.93 192.168.203.74; do
    rsync -a --delete \
...
Both `wan-native` (217.14.168.253) and `lan-downstream` (217.14.168.254) are
...
iperf3 traffic between tester01's sim-uplink (217.14.160.74/27, net zone) and
tester02's downstream (217.14.168.0/24, host zone), then re-run with
...
**Rule piggybacked**: `SSH(ACCEPT) net:217.14.160.64/27 $FW`
(line 47, `/etc/shorewall/rules` on the reference HA firewall primary node)

**Traffic path**: tester01 eth2 (`217.14.160.74/27`) → FW `bond1`
(`217.14.160.75`) tcp/22 → SSHd on the FW itself.  Zone: `net → $FW` (INPUT
...
`nic: eth2` (untagged, no VLAN), `ipv4: 217.14.160.74/27`.  This requires
...
1. `ssh root@192.168.203.93 "systemctl stop sim-uplink"` — sim-uplink uses
   eth2 (router ID `217.14.160.77`); must not conflict with the test endpoint.
2. `ssh root@192.168.203.93 "ip route"` — verify the default route still
...
4. `ssh root@192.168.203.93 "systemctl start sim-uplink"` — restore.
