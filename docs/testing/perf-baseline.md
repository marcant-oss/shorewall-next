...
scenario, which uses `wan-native` (native mode with IPv4 `217.14.168.253/24`).
...
1. `root@fw-primary` does not resolve (no DNS entry for this name in the
   test environment).  The correct address is `root@192.168.203.70`.
...
data:
- Update the catalogue entry: `fw_host: root@192.168.203.70`
- Deploy tester01's SSH public key to `root@192.168.203.70:~/.ssh/authorized_keys`
...
1. **Conntrack sidecar**: update `fw_host: root@fw-primary` to
   `fw_host: root@192.168.203.70` in `docs/testing/security-test-plan.ipv6-perf.yaml`
