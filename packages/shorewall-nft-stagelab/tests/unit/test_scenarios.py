...
def test_rule_scan_ipv6_uses_endpoint_ipv6() -> None:
    ...
    sc = RuleScanScenario(
        ...
        kind="rule_scan",
        source="probe-src",
        target_subnet="2a00:f88:0:3168::/64",
        random_count=3,
        family="ipv6",
    )
    cfg = _probe_cfg("2a00:f88:0:2000::200/64", [sc])
    ...
    for cmd in probe_cmds:
        assert cmd.spec["src_ip"] == "2a00:f88:0:2000::200"
    ...
    return StagelabConfig(
        ...
        endpoints=[
            Endpoint(
                ...
                ipv4="217.14.160.74/27",
                ipv4_gw="217.14.160.92",
            ),
        ],
        scenarios=scenarios,
        metrics=MetricsSpec(),
...
def test_conn_storm_direct_plan_single_command():
    ...
    assert storm_cmd.spec["target"] == "217.14.160.75:22"
    assert storm_cmd.spec["connections"] == 1000
    assert storm_cmd.spec["connect_rate"] == 200
...
def test_conn_storm_direct_summarize_records_peak():
    ...
    assert result.raw["target_ip"] == "217.14.160.75"
