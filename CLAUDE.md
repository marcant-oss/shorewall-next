...
- **192.168.203.83** — grml trixie/sid live, RAM-only.
...
13. **3-firewall config-merge replay + cleanup pass.** Re-do
    ...
    production firewalls (elgar, tropheus, third — IDENTIFY
    THE THIRD before starting; the local checkout under
    ``../elgar`` and ``../tropheus`` only ships two of them, the
    keepalived configs in ``../old/etc/keepalived/`` only carry
    ``@elgar`` / ``@tropheus`` / ``@marcant`` host-tag prefixes
    ...
    of the replay:
      ...
      * **Metadata pass** — every rule should carry a
        ...
        with provenance markers (``# from elgar:rules:1042``)
        so a future bisect can blame the origin.
      * **Discovered necessary rule adjustments** — the
        simlab green-run + the ``DROP:$LOG agfeo any``
    ...
    Pre-flight: walk ``../elgar/etc/netns.cfg/fw/`` and
    ``../tropheus/etc/netns.cfg/fw/`` for what's already
...
14. **shorewalld dnstap smoke harness on the ram-only VM.** A
    ...
    ``192.168.203.83``, clean stop via systemd-run. Useful
...
- Pre-existing rossini compiler bug: chain name
  `linux,vpn-voice` from a zone list with a comma. Surfaces on
  the rossini corpus config.
...
tools/setup-remote-test-host.sh root@192.168.203.83
