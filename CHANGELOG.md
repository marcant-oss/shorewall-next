...
- **compiler/ir._add_rule**: extends the existing ACCEPT /
  ...
  `DROP:$LOG agfeo any` in the rules file used to expand into
  every agfeo→X chain as an inline catch-all drop, and when
...
- **`?COMMENT` directive preservation** — `_emit_block` walks
  ...
  "Sophos UTM Administration", "CDN (dreamrobot.de)" etc.)
...
- Compiler: `_add_rule` now extends the existing ACCEPT/policy=
  ...
  `DROP:$LOG agfeo any` in `rules` used to expand into every
  agfeo→X chain as an inline catch-all drop, and when file order
