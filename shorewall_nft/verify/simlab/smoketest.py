...
def _build_probes(topo_tun_mac: dict) -> list:
    ...
    # rossini (net) → host:217.14.168.100:80 TCP SYN
    probes.append(ProbeSpec(
        probe_id=pid, inject_iface="bond1", expect_iface="bond0.20",
        payload=P.build_tcp(
            "217.14.160.69", "217.14.168.100", 80,
            dst_mac=topo_tun_mac.get("bond1"),
        ),
        match=m(proto="tcp", dst="217.14.168.100", dport=80),
    ))
    pid += 1
    ...
    probes.append(ProbeSpec(
        probe_id=pid, inject_iface="bond1", expect_iface="bond0.20",
        payload=P.build_udp(
            "217.14.160.69", "217.14.168.100", 53, sport=30000,
            dst_mac=topo_tun_mac.get("bond1"),
        ),
        match=m(proto="udp", dst="217.14.168.100", dport=53),
    ))
    pid += 1
    ...
    probes.append(ProbeSpec(
        ...
        payload=P.build_icmp(
            "217.14.160.69", "217.14.160.34",
            dst_mac=topo_tun_mac.get("bond1"),
        ),
        match=m(proto="icmp", dst="217.14.160.34"),
    ))
    pid += 1
